<x-wireui-modal name="uploadBrokerageNoteModal" width="2xl items-center sm:items-center" persistent x-on:close-modal="$closeModal('uploadBrokerageNoteModal')">
    <x-wireui-card title="{{ __('Upload Brokerage Note') }}" class="w-full md:w-2/3 lg:w-1/2">
        ...
        <label class="flex items-center justify-center col-span-1 bg-gray-100 shadow-md cursor-pointer md:col-span-2 dark:bg-secondary-700 rounded-xl h-64 @error('brokerageNotes') border border-red-500 @enderror">
        ...
        <div class="col-span-1 md:col-span-2 flex items-center mt-4">
            ...
            <div class="mx-3">
                <x-wireui-input placeholder="{{ __('Password') }}" type="{{ $shouldShowPasswords ? 'text' : 'password' }}" wire:model.live.debounce.300ms="defaultPassword" class="@error('defaultPassword') border-red-500 @enderror">
                    <x-slot name="append">
            ...
            <div class="mx-3">
                <select wire:model.live="defaultBroker" class="p-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-400 focus:border-primary-400 dark:bg-secondary-800 dark:text-gray-300 dark:border-gray-700 w-40
